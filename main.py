...
def process_style(call):
    ...
    try:
        ...
        output = client.run(
            ...
            input={"prompt": full_prompt, "aspect_ratio": "1:1", "output_format": "jpg"}
        )
        
        if output:
            image_url = output[0] if isinstance(output, list) else output
            caption_text = f"🍇 **Dream:**\n{user_text}\n\n🐈 **Style:** {style_name}"
        ...
        bot.send_message(call.message.chat.id, f"❌ خطا: {str(e)[:100]}")
    finally:
        if user_id in user_prompts: del user_prompts[user_id]

@bot.callback_query_handler(func=lambda call: call.data == "cancel")
def cancel_action(call):
    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text="❌ عملیات لغو شد.")
